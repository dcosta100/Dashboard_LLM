...
def pct_by_classification_split(dfin: pd.DataFrame, mode: str):
...
def _summarize_row(entry: dict, kind: str) -> str:
    """
    Resumo curto para exibir na célula (markdown). O JSON completo vai no tooltip.
    """
    if not isinstance(entry, dict) or not entry:
        return ""

    k = kind.lower()
    if k.startswith("meas"):  # Measurements
    ...
    if k.startswith("presc"):  # Prescriptions
...
# Barras empilhadas (LLM vs Parser) por classe
@app.callback(
...
def update_bar_section(modes_selected, this_id):
    mode = this_id["mode"]
    g = pct_by_classification_split(df, mode)
    g_long = g.melt(
        id_vars=["ClassificationType"],
        value_vars=["pct_llm", "pct_parser"],
        var_name="Fonte",
        value_name="% OK"
    )
    g_long["Fonte"] = g_long["Fonte"].map({"pct_llm": "LLM", "pct_parser": "Parser"})
    fig = px.bar(g_long, x="ClassificationType", y="% OK", color="Fonte", barmode="group",
...
# Tabelas de falhas/sucessos por modo
@app.callback(
